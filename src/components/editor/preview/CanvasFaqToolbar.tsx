...
// ─────────────────────────────────────────────────────────────
// Pearloom / editor/preview/CanvasFaqToolbar.tsx
//
// Floating dark-pill toolbar that appears when hovering an
// FAQ item on the canvas. Pattern mirrors CanvasChapterToolbar.
// ─────────────────────────────────────────────────────────────
...
export type FaqToolbarAction = 'edit' | 'moveUp' | 'moveDown' | 'delete';

interface CanvasFaqToolbarProps {
  rect: DOMRect;
  faqIndex: number;
  faqCount: number;
  onAction: (action: FaqToolbarAction) => void;
}
...
export function CanvasFaqToolbar({ rect, faqIndex, faqCount, onAction }: CanvasFaqToolbarProps) {
  ...
  return (
    <motion.div
      ...
      onMouseDown={stop}
      onClick={stop}
      onMouseEnter={() => window.dispatchEvent(new CustomEvent('pearloom-faq-hover-keep'))}
      style={{
        position: 'fixed',
        top,
        right,
