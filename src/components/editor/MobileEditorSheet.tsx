...
// Draggable snap sheet with radial FAB, story push navigation,
// keyboard avoidance, and arc backdrop
// ─────────────────────────────────────────────────────────────
...
function snapToNearest(y: number, snaps: { full: number; mid: number; peek: number }): number {
  ...
  return vals.reduce((closest, snap) =>
    Math.abs(snap - y) < Math.abs(closest - y) ? snap : closest
  );
}

// ── DragHandle row for Reorder.Item ───────────────────────────
...
export function MobileEditorSheet() {
  ...
  // ── Sheet Y motion value ──────────────────────────────────────
  const sheetY = useMotionValue(sheetH); // start offscreen
  ...
  const handleDragEnd = useCallback((_: unknown, info: { velocity: { y: number }; offset: { y: number } }) => {
    ...
    if (vy > 650) {
      // Fast downward flick: dismiss or snap down
      const next = current < snaps.mid ? snaps.mid : snaps.peek;
      if (next >= snaps.peek && current > 80) {
        snapTo(sheetH);
  ...
  // Title for header
  let headerTitle = TAB_LABELS[activeTab as EditorTab] ?? activeTab;
  ...
  function renderTabContent() {
    ...
    if (activeTab === 'canvas')  return <CanvasEditor manifest={manifest} onChange={actions.handleDesignChange} pushToPreview={actions.pushToPreview} />;
    if (activeTab === 'events')  return <EventsPanel manifest={manifest} onChange={actions.handleDesignChange} />;
    if (activeTab === 'design')  return <DesignPanel manifest={manifest} onChange={actions.handleDesignChange} />;
    if (activeTab === 'details') return <DetailsPanel manifest={manifest} onChange={actions.handleDesignChange} subdomain={subdomain} />;
    if (activeTab === 'pages')   return <PagesPanel manifest={manifest} subdomain={subdomain} onChange={actions.handleDesignChange} previewPage={state.previewPage} onPreviewPage={(slug) => dispatch({ type: 'SET_PREVIEW_PAGE', page: slug })} />;
    if (activeTab === 'blocks')  return <AIBlocksPanel manifest={manifest} coupleNames={['', '']} onChange={(m) => { actions.handleDesignChange(m); actions.pushToPreview(m); }} />;
    if (activeTab === 'voice')   return <VoiceTrainerPanel voiceSamples={manifest?.voiceSamples || []} onChange={(samples) => { const m = { ...manifest, voiceSamples: samples }; actions.handleDesignChange(m); actions.pushToPreview(m); }} />;
    return null;
  }
    ...
    <>
            ...
            return (
              <motion.button
                ...
                style={{
                  ...
                  borderRadius: 12,
                  border: isActive
                    ? '1.5px solid rgba(163,177,138,0.55)'
                  ...
                  color: isActive ? '#A3B18A' : 'rgba(214,198,168,0.75)',
                  cursor: 'pointer',
                  display: 'flex',
      ...
      >
        ...
        <div style={{
          ...
          {/* Back button (story editor subview) */}
          <AnimatePresence>
            {activeTab === 'story' && storySubview === 'editor' && (
              <motion.button
                ...
                style={{
                  ...
                  color: 'rgba(214,198,168,0.7)', cursor: 'pointer',
        ...
        <div style={{
          ...
          {([ 'story', 'canvas', 'events', 'design', 'details', 'blocks', 'voice' ] as EditorTab[]).map(tab => {
            const icons: Record<EditorTab, React.ElementType> = {
            ...
            const labels: Record<EditorTab, string> = {
              story: 'Story', canvas: 'Sections', events: 'Events',
              design: 'Design', details: 'Details', blocks: 'AI', voice: 'Voice', pages: 'Pages',
            };
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  cursor: 'pointer',
                  background: isActive ? 'rgba(214,198,168,0.08)' : 'transparent',
                  color: isActive ? 'rgba(214,198,168,0.92)' : 'rgba(214,198,168,0.3)',
                  position: 'relative',
                ...
                {isActive && (
                  <motion.div
                    ...
                    style={{
                      ...
                      right: '12%',
                      height: 2,
                      background: 'rgba(214,198,168,0.55)',
