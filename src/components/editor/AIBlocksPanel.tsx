...
const inp: React.CSSProperties = {
  ...
  border: '1px solid rgba(0,0,0,0.07)', background: '#F4F4F5',
...
function EventsEditor({ events, onChange }: { events: WeddingEvent[]; onChange: (e: WeddingEvent[]) => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      {events.map(evt => (
        <div key={evt.id} style={{ background: 'rgba(24,24,27,0.04)', borderRadius: '10px', border: '1px solid rgba(0,0,0,0.05)', overflow: 'hidden' }}>
          <button
            onClick={() => setExpanded(expanded === evt.id ? null : evt.id)}
            style={{ width: '100%', display: 'flex', alignItems: 'center', padding: '8px 10px', background: 'none', border: 'none', cursor: 'pointer', color: '#18181B', gap: '8px' }}
          >
            <span style={{ flex: 1, fontSize: '0.75rem', fontWeight: 700, textAlign: 'left' }}>{evt.name || 'Event'}</span>
            <span style={{ fontSize: '0.65rem', color: '#71717A' }}>{evt.time}</span>
              ...
              >
                <div style={{ padding: '0 12px 12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  <button
                    ...
                    style={{ alignSelf: 'flex-start', display: 'flex', alignItems: 'center', gap: '4px', padding: '4px 8px', borderRadius: '6px', border: '1px solid rgba(24,24,27,0.1)', background: 'rgba(24,24,27,0.05)', color: '#71717A', cursor: 'pointer', fontSize: '0.65rem', fontWeight: 700 }}
                  >
      ...
      <button onClick={add} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', padding: '8px', borderRadius: '8px', border: '1px dashed rgba(0,0,0,0.08)', background: 'transparent', color: '#3F3F46', cursor: 'pointer', fontSize: '0.65rem', fontWeight: 600 }}>
...
function FaqEditor({ faqs, onChange }: { faqs: FaqItem[]; onChange: (f: FaqItem[]) => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      {faqs.map((faq, i) => (
        <div key={faq.id} style={{ background: 'rgba(24,24,27,0.04)', borderRadius: '10px', border: '1px solid rgba(0,0,0,0.05)', padding: '8px 10px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
          <span style={{ fontSize: '0.6rem', color: '#71717A', fontWeight: 700, letterSpacing: '0.1em' }}>FAQ {i + 1}</span>
          ...
          <button onClick={() => remove(faq.id)} style={{ alignSelf: 'flex-start', display: 'flex', alignItems: 'center', gap: '4px', padding: '3px 6px', borderRadius: '4px', border: 'none', background: 'rgba(24,24,27,0.05)', color: '#71717A', cursor: 'pointer', fontSize: '0.6rem', fontWeight: 700 }}>
            <X size={9} /> Remove
          </button>
        </div>
      ))}
...
function TravelEditor({ travel, onChange }: { travel: TravelInfo; onChange: (t: TravelInfo) => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
      ...
      <div style={{ fontSize: '0.6rem', fontWeight: 800, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#71717A', marginTop: '4px' }}>Hotels</div>
      {(travel.hotels || []).map((h, i) => (
        <div key={i} style={{ background: 'rgba(24,24,27,0.04)', borderRadius: '10px', border: '1px solid rgba(0,0,0,0.05)', padding: '8px 10px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
          <span style={{ fontSize: '0.6rem', color: '#71717A', fontWeight: 700, letterSpacing: '0.1em' }}>Hotel {i + 1}</span>
          ...
          <button onClick={() => removeHotel(i)} style={{ alignSelf: 'flex-start', display: 'flex', alignItems: 'center', gap: '4px', padding: '3px 6px', borderRadius: '4px', border: 'none', background: 'rgba(24,24,27,0.05)', color: '#71717A', cursor: 'pointer', fontSize: '0.6rem', fontWeight: 700 }}>
            <X size={9} /> Remove
          </button>
        </div>
      ))}
      <button onClick={addHotel} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', padding: '8px', borderRadius: '8px', border: '1px dashed rgba(0,0,0,0.08)', background: 'transparent', color: '#3F3F46', cursor: 'pointer', fontSize: '0.65rem', fontWeight: 600 }}>
...
function RegistryEditor({ registry, onChange }: {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      ...
      <div style={{ fontSize: '0.6rem', fontWeight: 800, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#71717A', marginTop: '4px' }}>Registry Links</div>
      {entries.map((entry, i) => (
        <div key={i} style={{ background: 'rgba(24,24,27,0.04)', borderRadius: '10px', border: '1px solid rgba(0,0,0,0.05)', padding: '8px 10px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
          ...
          <button onClick={() => removeEntry(i)} style={{ alignSelf: 'flex-start', display: 'flex', alignItems: 'center', gap: '4px', padding: '3px 6px', borderRadius: '4px', border: 'none', background: 'rgba(24,24,27,0.05)', color: '#71717A', cursor: 'pointer', fontSize: '0.6rem', fontWeight: 700 }}>
            <X size={9} /> Remove
          </button>
        </div>
      ))}
      <button onClick={addEntry} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', padding: '8px', borderRadius: '8px', border: '1px dashed rgba(0,0,0,0.08)', background: 'transparent', color: '#3F3F46', cursor: 'pointer', fontSize: '0.65rem', fontWeight: 600 }}>
...
function BlockCard({
  ...
  return (
    <motion.div
      layout
      style={{
        background: 'rgba(24,24,27,0.04)', borderRadius: '12px',
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#18181B' }}>{block.label}</span>
            {isApplied && (
              <span style={{ fontSize: '0.6rem', fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', color: block.color, background: `${block.color}15`, padding: '2px 6px', borderRadius: '4px' }}>
                Active
              </span>
            )}
          </div>
          <div style={{ fontSize: '0.65rem', color: '#71717A', lineHeight: 1.4, marginTop: '2px' }}>
            {block.description}
          </div>
        </div>
            ...
            <div style={{ padding: '0 12px 12px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <div>
                ...
                <textarea
                  ...
                  style={{ ...inp, resize: 'vertical', lineHeight: 1.55, fontSize: '0.75rem' }}
              ...
              <motion.button
                ...
                style={{
                  ...
                  fontSize: '0.75rem', fontWeight: 700, cursor: isGenerating ? 'not-allowed' : 'pointer',
              ...
              {isApplied && (
                <button
                  ...
                  style={{
                    ...
                    color: block.color, fontSize: '0.65rem', fontWeight: 700, cursor: 'pointer',
                  }}
                >
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px' }}>
        <div>
          <div style={{ fontSize: '0.6rem', fontWeight: 800, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#71717A', marginBottom: '0.3rem' }}>
            Auto-Fill Sections
          </div>
          <p style={{ fontSize: '0.7rem', color: '#71717A', lineHeight: 1.5, margin: 0 }}>
      ...
      <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
        ...
        >
          All
          <span style={{ background: activeCategory === 'all' ? '#71717A' : 'rgba(0,0,0,0.06)', padding: '1px 5px', borderRadius: '8px', fontSize: '0.6rem', fontWeight: 800 }}>
            {BLOCKS.length}
          </span>
        </button>
        {BLOCKS.map(block => {
          ...
          return (
            <button
              ...
              style={{
                ...
                color: isActive ? '#fff' : isDone ? block.color : '#3F3F46',
                fontSize: '0.6rem', fontWeight: 700, transition: 'all 0.15s',
              }}
            >
              <Icon size={10} color={isActive ? '#fff' : isDone ? block.color : '#3F3F46'} />
              {block.label}
              {isDone && (
                <span style={{ background: isActive ? '#71717A' : `${block.color}25`, padding: '1px 4px', borderRadius: '6px', fontSize: '0.6rem', fontWeight: 800 }}>
                  <Check size={7} />
                </span>
              )}
            </button>
      ...
      {error && (
        <div style={{ background: 'rgba(24,24,27,0.06)', border: '1px solid rgba(24,24,27,0.1)', borderRadius: '8px', padding: '8px 12px', fontSize: '0.75rem', color: '#71717A', display: 'flex', alignItems: 'center', gap: '6px' }}>
          <X size={12} /> {error}
          <button onClick={() => setError(null)} style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', color: '#71717A' }}><X size={10} /></button>
        </div>
      )}
      ...
      {generatedCount > 0 && (
        <div style={{
          ...
          background: 'rgba(24,24,27,0.04)', border: '1px solid rgba(24,24,27,0.08)',
        }}>
          <Check size={12} color="#4ade80" />
          <span style={{ fontSize: '0.65rem', fontWeight: 700, color: '#18181B' }}>
      ...
      <AnimatePresence mode="popLayout">
        {activeEdit && (
          <motion.div
            ...
            transition={{ duration: 0.22 }}
            style={{
              background: 'rgba(24,24,27,0.03)', borderRadius: '12px',
              border: '1px solid rgba(0,0,0,0.06)', padding: '1rem',
            }}
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
              <span style={{ fontSize: '0.65rem', fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase', color: BLOCKS.find(b => b.id === activeEdit)?.color }}>
