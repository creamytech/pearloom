...
export function AuthModal({ open, onClose }: AuthModalProps) {
  ...
  // Phase 4.1 of AUDIT-2026-05-29 — a11y. Title id wired into
  // aria-labelledby on the dialog so screen readers announce the
  // modal as "Create your account" / "Welcome back" on open.
  const titleId = useId();

  // Esc dismisses — same contract every other Pearloom modal
  // follows (matches DesignAdvisor, DashSettings delete dialog).
  useEffect(() => {
    if (!open) return;
      ...
      >
        ...
        >
          ...
          <div className="px-7 pt-7 pb-4">
            ...
            <h2 id={titleId} className="font-heading text-[1.5rem] text-[var(--pl-ink)] mt-2 leading-tight" style={{ fontWeight: 400 }}>
          ...
          <div className="px-7 pb-7">
            ...
            {error && (
              <div role="alert" className="mb-4 px-3 py-2 rounded-lg text-[0.78rem] text-red-700" style={{ background: 'rgba(220,38,38,0.06)', border: '1px solid rgba(220,38,38,0.12)' }}>
                {error}
              </div>
            )}
            ...
            <p className="text-center text-[0.78rem] text-[var(--pl-muted)] mt-5">
              {mode === 'register' ? (
                <>Already have an account?{' '}
                  <button type="button" onClick={() => { setMode('login'); setError(''); }} className="bg-transparent border-none cursor-pointer text-[var(--pl-olive)] font-semibold underline">
                    Sign in
                  </button>
                </>
              ) : (
                <>Don&apos;t have an account?{' '}
                  <button type="button" onClick={() => { setMode('register'); setError(''); }} className="bg-transparent border-none cursor-pointer text-[var(--pl-olive)] font-semibold underline">
                    Create one
                  </button>
                </>
              )}
