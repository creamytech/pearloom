...
import { renderPostmark } from '@/lib/invite-engine/render';
...
export async function POST(req: NextRequest) {
  ...
  const rate = checkRateLimit(`invite-postmark:${session.user.email}:${getClientIp(req)}`, { max: 12, windowMs: 5 * 60_000 });
  ...
  if (!body.siteSlug || !body.names || !body.palette || !body.date) {
  ...
  try {
    const result = await renderPostmark({ ctx, siteSlug: body.siteSlug });
    if (!result) return NextResponse.json({ error: 'Renderer returned nothing.' }, { status: 502 });
    return NextResponse.json({ ok: true, ...result });
  } catch (err) {
    console.error('[invite/postmark] failed:', err);
