...
async function extractExifGps(imageUrl: string): Promise<{ lat: number; lng: number } | null> {
  try {
    ...
    if (!res.ok && res.status !== 206) {
      ...
      const fullRes = await fetch(imageUrl, { signal: AbortSignal.timeout(10000) });
...
async function aiGuessLocation(imageUrl: string, context?: string): Promise<string> {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Resolve proxied URLs back to the real URL
    ...
    const gps = await extractExifGps(resolvedUrl);
    if (gps) {
    ...
    const aiLocation = await aiGuessLocation(resolvedUrl, occasion);
    if (aiLocation) {
      return NextResponse.json({
        location: aiLocation,
        method: 'ai-vision',
