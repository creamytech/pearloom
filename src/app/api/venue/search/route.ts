...
export async function GET(req: NextRequest) {
  ...
  if (type === 'autocomplete') {
    ...
    if (!apiKey) {
      console.error('[venue/search] GOOGLE_PLACES_API_KEY is not set — venue search will not work');
      return NextResponse.json({ predictions: [], error: 'Venue search not configured — set GOOGLE_PLACES_API_KEY' });
    }
  ...
  if (type === 'details') {
    ...
    if (!apiKey) {
      return NextResponse.json({ place: null, error: 'GOOGLE_PLACES_API_KEY not configured' }, { status: 500 });
    }
