...
| **O3 — the awaited arrival** — **SHIPPED 2026-07-08** | M1: the arrival sheet is addressed when requests await ("Scott is keeping you a seat." — olive italic, fade-late; the overture breathes longer). M4: a "Your people" movement appears after the mark ONLY when `pendingIncoming` is non-empty (fetched from /api/friends on mount; frozen once the walker passes the mark so a slow fetch never shifts the ground) — each request a SEALED NOTE with a wax seal carrying the sender's initial; "Break the seal" opens it ("Scott would keep you in their circle" · first-names-only note) → "Keep them" accepts via the API, "Not now" just re-seals (stays pending in their Circle — a welcome, not a judgment). Folio numbers went positional so the dynamic movement can't collide. `?people=1` on /dev/welcome seeds the envelopes for QA. *Deferred within O3:* the guest-row claim context ("Shaun & Scott's WEDDING awaits you") — needs a sites-by-guest-email endpoint; the circle-invite context ships now because S1 made it free. | 1 session |
