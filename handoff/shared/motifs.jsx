/* ========================================================================
   PEARLOOM MOTIFS — the shared visual language
   Pear, stamps, blobs, squiggles, sparkles, hearts, post-its, polaroids
   ======================================================================== */

const { useId } = React;

/* ---------- Pear (leaf + shadow + stem, as spec'd) ---------- */
function Pear({ size = 56, tone = 'sage', shadow = true, sparkle = false, className = '' }) {
  const id = useId();
  const tones = {
  ...
  return (
    <svg viewBox="0 0 100 120" width={size} height={size * 1.2} className={className} aria-hidden="true">
      ...
      {/* leaf */}
      <path
        d="M54 4 C 64 2, 74 8, 74 18 C 66 20, 56 16, 54 4 Z"
        fill={t.leaf}
      />
      {/* highlight */}
...
/* ---------- Pear Mascot — the cute face version ---------- */
function PearMascot({ size = 120, mood = 'happy', className = '', style = {} }) {
  // moods: happy | wink | love | sleep | wave
  return (
    <svg viewBox="0 0 120 140" width={size} height={size * (140/120)} className={className} style={style} aria-hidden="true">
      {/* shadow */}
...
/* ---------- Pearloom Logo (wordmark + pear) ---------- */
function PearloomLogo({ size = 28 }) {
  return (
    <div className="logo">
      <Pear size={size + 8} tone="sage" shadow={false} />
      <span style={{ fontSize: size * 0.9, lineHeight: 1, display: 'inline-flex', alignItems: 'center', gap: 3 }}>
        Pearloom
        <svg width={size * 0.28} height={size * 0.28} viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2 L13 10 L22 12 L13 14 L12 22 L11 14 L2 12 L11 10 Z" fill="var(--gold)"/></svg>
      </span>
    </div>
  );
}

/* ---------- Circular Stamp — "MADE FOR MEANINGFUL MOMENTS" ---------- */
function Stamp({
  size = 100,
  tone = 'lavender',   // lavender | peach | sage | cream
  text = 'MADE FOR MEANINGFUL MOMENTS',
  icon = 'pear',       // pear | heart | sparkle
  rotation = -8,
  className = '',
}) {
  const id = useId();
    ...
    <div className={className} style={{
      ...
      <svg viewBox="0 0 100 100" width={size} height={size} aria-hidden="true">
        ...
        <text fill={t.ink} fontSize="9" fontWeight="700" fontFamily="Inter, sans-serif" letterSpacing="1.2">
          <textPath href={`#${id}-arc`} startOffset="2%">{text} · {text} ·</textPath>
        </text>
      </svg>
      <div style={{
        ...
        {icon === 'heart' && (
          <svg viewBox="0 0 24 24" width={size * 0.3} height={size * 0.3}>
            <path d="M12 21s-7-4.5-9.5-9C.5 8 3 4 7 4c2 0 3.5 1 5 3 1.5-2 3-3 5-3 4 0 6.5 4 4.5 8-2.5 4.5-9.5 9-9.5 9z" fill="#3D4A1F" />
          </svg>
        )}
        {icon === 'sparkle' && (
...
/* ---------- Loopy orange squiggle ---------- */
function Squiggle({ width = 200, height = 80, variant = 0, stroke = '#D4A95D', className = '', style = {} }) {
  const paths = [
    // simple wave
  ...
  return (
    <svg viewBox="0 0 200 80" width={width} height={height} className={className} style={style} aria-hidden="true">
...
/* ---------- Sparkle glyph ---------- */
function Sparkle({ size = 14, color = '#D4A95D', className = '', style = {} }) {
  return (
...
/* ---------- Heart glyph ---------- */
function Heart({ size = 16, color = '#E8A07A', className = '', style = {} }) {
  return (
    <svg viewBox="0 0 24 24" width={size} height={size} className={className} style={style} aria-hidden="true">
      <path d="M12 21s-7-4.5-9.5-9C.5 8 3 4 7 4c2 0 3.5 1 5 3 1.5-2 3-3 5-3 4 0 6.5 4 4.5 8-2.5 4.5-9.5 9-9.5 9z" fill={color} />
    </svg>
  );
}
...
function PostIt({
  ...
  const bgs = { lavender: '#C4B5D9', peach: '#F0C9A8', cream: '#F3E9D4', sage: '#CBD29E' };
  return (
    <div className={className} style={{
      width,
  ...
  return (
    <div className={className} style={{
      ...
      <div style={{
        width: '100%',
        aspectRatio: '1 / 1',
        background: `#e8e4d5 url(${src}) center/cover no-repeat`,
      }} role="img" aria-label={alt} />
      {caption && (
        <div style={{
          position: 'absolute', bottom: 8, left: 0, right: 0,
          textAlign: 'center', fontFamily: 'var(--font-script)',
...
/* ---------- Placeholder photo (when we don't have real imagery) ---------- */
function PhotoPlaceholder({
  ...
  return (
    <div
      className={className}
      style={{
        width, aspectRatio: aspect,
        background: src
          ? `#e8e4d5 url(${src}) center/cover no-repeat`
          : (bgs[tone] || bgs.lavender),
        display: 'grid', placeItems: 'center',
...
function Atmosphere({ preset = 'warm', className = '', style = {} }) {
  const presets = {
    warm: [
      { el: 'blob', tone: 'lavender', size: 480, opacity: 0.7, style: { top: -80, left: -100, transform: 'rotate(20deg)' } },
      { el: 'blob', tone: 'peach', size: 420, opacity: 0.6, style: { top: 40, right: -120, transform: 'rotate(-30deg)' } },
      { el: 'blob', tone: 'sage', size: 360, opacity: 0.5, style: { bottom: -100, left: 40, transform: 'rotate(-15deg)' } },
      { el: 'squig', variant: 1, width: 260, style: { top: 120, right: 200, transform: 'rotate(-20deg)' } },
      { el: 'squig', variant: 2, width: 180, style: { bottom: 120, left: 180, transform: 'rotate(30deg)' } },
    ],
    hero: [
      { el: 'blob', tone: 'lavender', size: 520, opacity: 0.55, style: { top: -120, left: -100 } },
      { el: 'blob', tone: 'peach', size: 440, opacity: 0.5, style: { top: 60, right: -120 } },
      { el: 'blob', tone: 'sage', size: 380, opacity: 0.35, style: { bottom: -140, right: 100 } },
      { el: 'squig', variant: 1, width: 240, style: { top: 80, right: 300, transform: 'rotate(12deg)' } },
      { el: 'squig', variant: 3, width: 180, style: { top: 260, left: 40, transform: 'rotate(-15deg)' } },
    ],
    sparse: [
      { el: 'blob', tone: 'lavender', size: 300, opacity: 0.4, style: { top: 40, left: -60 } },
      { el: 'blob', tone: 'peach', size: 240, opacity: 0.3, style: { bottom: -40, right: 40 } },
      { el: 'squig', variant: 0, width: 200, style: { top: 100, right: 80, transform: 'rotate(8deg)' } },
    ],
  };
  ...
  return (
    <div className={`atmosphere ${className}`} style={style}>
      {items.map((it, i) => {
        if (it.el === 'blob') return <Blob key={i} tone={it.tone} size={it.size} opacity={it.opacity} seed={i} style={it.style} />;
        if (it.el === 'squig') return <Squiggle key={i} variant={it.variant} width={it.width} height={it.width * 0.4} style={it.style} />;
        return null;
      })}
    </div>
  );
...
/* ---------- Footer sticker — olive pill with pear & script ---------- */
function FooterSticker({ children = "It's free to get started!", size = 1, className = '', style = {} }) {
  const w = 280 * size, h = 110 * size;
  return (
    <div className={className} style={{ position: 'relative', width: w, height: h, ...style }}>
      <svg viewBox="0 0 280 110" width={w} height={h} aria-hidden="true" style={{ position: 'absolute', inset: 0 }}>
        {/* outer cream border */}
        <path
          d="M 20 30 Q 20 10, 40 10 L 240 10 Q 264 10, 262 32 L 266 60 Q 262 88, 244 92 L 56 100 Q 24 104, 20 80 Q 16 60, 20 30 Z"
          fill="#FDFAF0" stroke="none"
        />
        {/* olive inner */}
        <path
          d="M 26 34 Q 26 16, 44 16 L 236 16 Q 258 16, 256 34 L 260 60 Q 256 84, 240 88 L 58 94 Q 30 98, 26 78 Q 22 60, 26 34 Z"
          fill="#3D4A1F"
        />
        {/* small sparkle near right */}
        <path d="M230 32 L232 38 L238 40 L232 42 L230 48 L228 42 L222 40 L228 38 Z" fill="#F8F1E4" opacity="0.85"/>
        {/* small daisy near pear */}
        <g transform="translate(115 56)">
          <circle r="3" fill="#F8F1E4"/>
          <circle cx="-6" cy="0" r="2" fill="#F8F1E4"/><circle cx="6" cy="0" r="2" fill="#F8F1E4"/>
          <circle cx="0" cy="-6" r="2" fill="#F8F1E4"/><circle cx="0" cy="6" r="2" fill="#F8F1E4"/>
        </g>
      </svg>
      {/* pear mascot on the left */}
      <div style={{ position: 'absolute', left: 14 * size, top: 4 * size, width: 78 * size, height: 92 * size }}>
        <PearMascot size={78 * size} mood="happy" />
      </div>
      {/* script text */}
      <div style={{
        position: 'absolute', right: 28 * size, top: 28 * size,
        fontFamily: 'var(--font-script)', fontSize: 28 * size,
        color: '#F8F1E4', lineHeight: 1, textAlign: 'center',
      }}>
...
function FootBar({ primaryText = 'Made for meaningful moments.', ctaText = 'Create your event', onCta, extras }) {
  return (
      ...
      <div className="footbar-inner">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2, marginRight: 'auto' }}>
          <div style={{
            ...
            fontSize: 22, fontWeight: 600, color: 'var(--cream)',
          }}>
            {primaryText}
          </div>
        ...
        {extras}
        {ctaText && (
          <button className="btn" style={{ background: 'var(--cream)', color: 'var(--ink)' }} onClick={onCta}>
            {ctaText}
            <Pear size={14} tone="ink" shadow={false} />
          </button>
        )}
        <div style={{
          fontFamily: 'var(--font-script)', fontSize: 18, color: 'var(--cream)',
          opacity: 0.85, whiteSpace: 'nowrap',
        }}>
          It's free <br/>to get started!
        </div>
...
/* ---------- Top nav (marketing) ---------- */
function TopNav({ active, links, ctaText = 'Start your timeline', onCta, loggedIn = false }) {
  const defaultLinks = links || [
    { label: 'Product', href: '#', hasMenu: true },
    { label: 'Event Types', href: '#', hasMenu: true },
    { label: 'Features', href: '#', hasMenu: true },
    { label: 'Examples', href: 'Pearloom Templates.html' },
    { label: 'Pricing', href: '#' },
    { label: 'Resources', href: '#', hasMenu: true },
  ];
  return (
    <div className="topnav">
      <div className="topnav-inner">
        ...
        <nav className="nav-links">
          {defaultLinks.map(l => (
            <a key={l.label} href={l.href} className={active === l.label ? 'active' : ''}>
              {l.label}
              {l.hasMenu && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ opacity: 0.5 }}><path d="M6 9l6 6 6-6"/></svg>}
            </a>
          ))}
        </nav>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {loggedIn ? (
            <a className="btn btn-ghost" href="Pearloom Home Redesign.html">Dashboard</a>
          ) : (
            <a className="btn btn-ghost" href="Pearloom Signin.html">Log in</a>
          )}
...
/* ---------- Custom icon set — one place, zero emojis ----------
   Usage: <Icon name="arrow-right" size={16} />
   Always strokeWidth 1.75, rounded caps, currentColor — they read crisp at 14-20px.
*/
function Icon({ name, size = 16, color = 'currentColor', className = '', style = {}, strokeWidth = 1.75 }) {
  const common = {
...
/* ---------- Export globally for other Babel scripts ---------- */
Object.assign(window, {
  Pear, PearMascot, PearloomLogo, Stamp, Blob, Squiggle, Sparkle, Heart,
  PostIt, Polaroid, PhotoPlaceholder, Atmosphere, FootBar, FooterSticker, TopNav,
  Icon,
});
