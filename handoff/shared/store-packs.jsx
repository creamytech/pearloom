/* =========================================================================
   PEARLOOM THEME STORE — the pack catalog.
   Each pack is a full, real theme (same --t-* var shape as shared/themes.jsx)
   plus store metadata (collection, price, tier, rating, badges, tags) and the
   recommended component kit. Built through a factory so previews render with
   the live procedural texture/motif/type engine — real art, not mockups.

   Exports (window): STORE_PACKS, STORE_COLLECTIONS, getPack, dividerForMotif
   ========================================================================= */
...
const S = { caveat: "'Caveat', cursive", dancing: "'Dancing Script', cursive", pinyon: "'Pinyon Script', cursive" };

function cm(a, pct, b) { return `color-mix(in oklab, ${a} ${pct}%, ${b})`; }
...
function mk(o) {
  ...
  const vars = {
    '--t-paper': paper, '--t-section': section, '--t-card': card,
    '--t-ink': ink, '--t-ink-soft': o.inkSoft || cm(ink, 74, paper), '--t-ink-muted': o.inkMuted || cm(ink, 48, paper),
    ...
    '--t-display': o.display || F.fraunces, '--t-body': o.body || F.inter, '--t-script': o.script || S.caveat,
  ...
  return {
    id: o.id, name: o.name, collection: o.collection, blurb: o.blurb,
    vars, texture: o.texture || 'none', pattern: o.pattern || 'none', motif: o.motif || 'none', kit: o.kit || 'classic',
    dark, foil: !!o.foil,
    swatches: o.swatches || [accent, dark ? (o.accent2 || gold) : ink, gold, section],
    price, tier, rating: o.r || 4.8, sales: o.s || '1.0k',
    badges: o.badges || {}, tags: o.tags || [],
    look: o.look || {},
  };
}
...
const STORE_PACKS = [
  ...
  mk({ id: 'ivory-minimal', name: 'Ivory Minimal', collection: 'modern', blurb: 'Barely-there ivory & soft taupe.', paper: '#F8F6F0', ink: '#3A352C', accent: '#A89578', accent2: '#C8BBA2', gold: '#B79E72', display: F.italiana, body: F.dmsans, script: F.dancing, radius: 3, ls: '0.24em', texture: 'none', motif: 'none', kit: 'minimal', price: 16, r: 4.9, s: '2.2k', badges: { best: true }, tags: ['ivory','taupe','quiet'] }),
  ...
  /* ===== Patterns & Prints (new vocabulary) ===== */
...
function getPack(id) { return STORE_PACKS.find(p => p.id === id) || STORE_PACKS[0]; }
function dividerForMotif(motif) { return ({ olive: 'sprig', bloom: 'brush', pressed: 'dot', none: 'rule' })[motif] || 'rule'; }

Object.assign(window, { STORE_PACKS, STORE_COLLECTIONS, getPack, dividerForMotif });
